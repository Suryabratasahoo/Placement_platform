...
interface BarData { day: string; percentage: number; val: number; active: boolean }
...
export default function StudentApplicationsPage() {
    ...
    const [stats, setStats] = useState({ totalApplied: 0, totalShortlisted: 0 });
    ...
    useEffect(() => {
        const fetchTrackerData = async () => {
            try {
                if (!userId) return;
                setIsLoading(true);
                const res = await fetch(`/api/student/applications?userId=${userId}`);
                if (res.ok) {
                    const data = await res.json();
                    setBarData(data.barData);
                    setPieData(data.pieData);
                    setThisWeekTotal(data.thisWeekTotal);
                    setStats(data.stats);
                } else {
                    toast.error("Failed to load application activity.");
                }
            } catch (error) {
                ...
                >
                    ...
                    <div className="lg:col-span-3 flex flex-col gap-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { label: "Applied", value: stats.totalApplied, color: "bg-[#fdecdb]", iconColor: "#e67b2f", icon: <Briefcase size={16} /> },
                                { label: "Shortlisted", value: stats.totalShortlisted, color: "bg-[#e2e0f7]", iconColor: "#681cf5", icon: <Star size={16} /> },
                            ].map((s, i) => (
                    ...
                    <div className="lg:col-span-9 bg-[#f5f5f5] rounded-[50px] p-10 flex flex-col gap-6 overflow-y-auto scrollbar-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                        ...
                        <div className="bg-[#FFA365] rounded-[45px] p-10 relative">
                            <div className="flex gap-12 mb-10 relative z-10 text-black font-sans">
                                <h3 className="text-4xl font-bold">{thisWeekTotal} <span className="text-lg font-medium opacity-70">apps sent</span></h3>
                                <h3 className="text-4xl font-bold">{stats.totalShortlisted} <span className="text-lg font-medium opacity-70">responses</span></h3>
                            </div>
